...
// Detailed sample clients list matching reference screenshot exactly
...
export default function ClientsPage() {
  ...
  const filteredClients = clients.filter(c => {
    ...
    const matchesLocation = selectedLocation === 'ALL' || c.location.includes(selectedLocation)
  ...
  const uniqueLocations = Array.from(new Set(clients.map(c => c.location.split(',')[0])))
    ...
    <div className="space-y-5 pb-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
        <div>
          ...
          <p className="text-xs font-medium text-slate-500 mt-1.5 max-w-2xl">
            Manage your enterprise client accounts, contacts, and deal opportunities.
          </p>
        </div>
      ...
      <div className="bg-white rounded-3xl p-4 border border-slate-200/80 shadow-2xs flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3.5">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="Search clients by name, industry, city, email..."
            value={searchQuery}
      ...
      {/* 4. Client Cards Grid / Table Section */}
      {viewMode === 'grid' ? (
                ...
                <div>
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex items-center gap-3">
                      <div className={`w-11 h-11 rounded-2xl ${client.avatarBg} text-white font-black text-lg flex items-center justify-center shrink-0 shadow-2xs`}>
                        {client.avatarLetter}
                      </div>
                      <div>
                  ...
                  <div className="grid grid-cols-2 gap-2.5 pt-2 border-t border-slate-100 text-[11px] font-bold text-slate-600">
                  ...
                  <tr key={client.id} className="hover:bg-amber-50/20 transition-colors group">
                    <td className="py-4 px-5 font-extrabold text-slate-900 group-hover:text-amber-800 transition-colors">
                      <div className="flex items-center gap-2.5">
                        <div className={`w-7 h-7 rounded-lg ${client.avatarBg} text-white font-black text-xs flex items-center justify-center shrink-0`}>
                          {client.avatarLetter}
                        </div>
                        <div>
                    ...
                    <td className="py-4 px-5 font-semibold text-slate-600">{client.industry}</td>
                    <td className="py-4 px-5 font-semibold text-slate-500">{client.location}</td>
                    <td className="py-4 px-5 font-medium text-slate-500">{client.email}</td>
                    <td className="py-4 px-5">
                      <span className={`px-2.5 py-1 text-[10px] font-extrabold rounded-md whitespace-nowrap ${badge.class}`}>
                        {badge.text}
                      </span>
                    </td>
                    <td className="py-4 px-5 font-bold text-slate-700">{client.proposalsCount} Proposals</td>
                    <td className="py-4 px-5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => handleOpenEditModal(client)}
                          className="px-3 py-1 bg-slate-100 hover:bg-[#FFC800] text-slate-800 hover:text-slate-950 font-bold rounded-lg text-xs transition-colors"
                        >
                          Edit
                        </button>
                        <button
          ...
          <div className="bg-white rounded-3xl max-w-xl w-full p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-150 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateClient} className="space-y-4 text-xs font-bold text-slate-700">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block mb-1">Pipeline Value</label>
                  <input
                    type="text"
